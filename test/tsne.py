...
def test_iris():
    iris = load_iris()
    X = iris.data
    assertEqual(mean_shift(X)[0], 2)
    Y = tsne(X)
    plt.scatter(Y[:, 0], Y[:, 1], c=iris.target)

    num_clusters, cluster_centers = mean_shift(Y)
    assertTrue(num_clusters > 1)
    assertTrue(num_clusters < 4)
...
def test_set_rand_seed():
    ...
    assertEqual(round(Y_a[0][0] / 5), round(Y_b[0][0] / 5))
    assertEqual(round(Y_a[0][1] / 5), round(Y_b[0][1] / 5))
...
def test_without_seed_positions():
    iris = load_iris()
...
def test_seed_positions():
    ...
    assertEqual(round(Y_a[0][0] / 20), round(Y_b[0][0] / 20))
    assertEqual(round(Y_a[0][1] / 20), round(Y_b[0][1] / 20))
